...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    if (isAuthenticated) {
      if (authUser?.role === 'teacher') {
        console.log('👨‍🏫 老師身份，重定向到教師頁面')
        router.replace('/teacher')
        return
      }
      console.log('✅ 用戶已認證（學生），允許訪問應用首頁')
    }
  }, [authLoading, needsRegistration, isAuthenticated, authUser?.role, router])
  ...
  const renderContent = () => {
    ...
    if (activeTab === "profile") {
      return <ProfileContent user={user} onUserChange={setUser} lineUserId={lineUserId} />
    }
  ...
  // 老師身份的首頁閃屏防護：避免在重定向前渲染學生頁內容
  if (isAuthenticated && authUser?.role === 'teacher') {
    return (
      <div className="flex items-center justify-center min-h-screen">
        <div className="text-center">
          <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-gray-900 mx-auto"></div>
          <p className="text-gray-600 mt-4">正在跳轉到教師頁面…</p>
        </div>
      </div>
    )
  }
