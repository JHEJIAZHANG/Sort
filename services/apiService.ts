...
export class ApiService {
  ...
  static bootstrapLineUserId(): string {
    ...
    try {
      if (typeof window !== 'undefined') {
        ...
        const fromUrl = params.get('line_user_id') || params.get('lineUserId') || ''
        if (fromUrl && fromUrl.trim()) {
          this.setLineUserId(fromUrl.trim())
          return this.lineUserId
        }
        const cookies = document.cookie.split(';')
        for (let i = 0; i < cookies.length; i++) {
          const cookie = cookies[i].trim()
          if (cookie.startsWith('line_user_id=')) {
            const val = decodeURIComponent(cookie.substring('line_user_id='.length))
            if (val && val.trim()) {
              this.lineUserId = val.trim()
              return this.lineUserId
            }
          }
        }
  ...
  private static async request<T = any>(
    ...
    try {
      ...
      let baseUrl: string
      if (typeof window !== 'undefined') {
        const rawPublic = (process.env.NEXT_PUBLIC_API_URL || '').replace(/\/+$/, '')
        const lower = rawPublic.toLowerCase()
        if (rawPublic) {
          if (apiPrefix === 'oauth') {
            baseUrl = /\/api\/oauth$/.test(lower) ? rawPublic : `${rawPublic}/api/oauth`
          } else if (apiPrefix === 'onboard' || apiPrefix === 'other') {
            baseUrl = /\/api$/.test(lower) ? rawPublic : `${rawPublic}/api`
          } else {
            baseUrl = /\/api\/v2$/.test(lower) ? rawPublic : `${rawPublic}/api/v2`
          }
        } else {
          if (apiPrefix === 'oauth') baseUrl = '/api/oauth'
          else if (apiPrefix === 'onboard' || apiPrefix === 'other') baseUrl = '/api'
          else baseUrl = '/api/v2'
        }
      } else {
        // 伺服器端，使用環境變數
  ...
  static async getMyPayments() {
    return this.request('/me/payments')
  }
  ...
  static async importTimetableImage(image: File, options?: { preview?: boolean; dryRun?: boolean; role?: 'student' | 'teacher'; mode?: string }) {
    if (!this.lineUserId) {
      this.bootstrapLineUserId()
    }
    const formData = new FormData()
  ...
  static async teacherOcrPreview(image: File) {
    return this.importTimetableImage(image, { preview: true, role: 'teacher', mode: 'teacher' })
  }

  static async teacherOcrConfirm(courses: any[], options?: { autoCreateClassroom?: boolean }) {
    if (!this.lineUserId) {
      this.bootstrapLineUserId()
    }
    const payload = {
      line_user_id: this.lineUserId,
      courses,
      auto_create_classroom: options?.autoCreateClassroom ?? true
    }
    return this.request('/classroom/teacher/ocr-import/', {
      method: 'POST',
      body: JSON.stringify(payload)
    }, 'other')
  }
  ...
  // LINE Login OAuth 相關 API
  static async getLineOAuthUrl() {
    if (typeof window !== 'undefined') {
      try { await fetchCsrfToken('') } catch { }
    }
    return this.request<{ auth_url: string }>('/line/url/', {
      method: 'POST'
    }, 'oauth')
  }
  ...
  static async getCourseMemberStatus(courseId: string) {
    if (!this.lineUserId) {
      this.bootstrapLineUserId()
    }
    ...
    return this.request(`/teacher/courses/${courseId}/member-status/?${params}`, {
      method: 'GET'
    }, 'other')
  }
  ...
  static async sendClassroomReminder(courseId: string, studentIds: string[]) {
    if (!this.lineUserId) {
      this.bootstrapLineUserId()
    }
    ...
    return this.request(`/teacher/courses/${courseId}/remind-classroom/`, {
      method: 'POST',
      body: JSON.stringify(payload)
    }, 'other')
  }

  // 第一個 sendAssignmentReminder 函數已刪除（參數格式錯誤）
  // 保留下方第二個版本（使用正確的 coursework_id 和 course_id 參數）
  ...
  static async refreshLineGroupMembers(courseId: string, groupId: string) {
    if (!this.lineUserId) {
      this.bootstrapLineUserId()
    }
    return this.request(`/teacher/courses/${courseId}/line-groups/${groupId}/refresh/`, {
      method: 'POST',
      body: JSON.stringify({
        line_user_id: this.lineUserId,
        course_id: courseId,
  ...
  static async getAssignmentsSubmissionStatus(courseId: string, assignmentIds: string[]) {
    if (!this.lineUserId) {
      this.bootstrapLineUserId()
    }
    const payload = {
      line_user_id: this.lineUserId,
      course_coursework_pairs: assignmentIds.map(id => ({ course_id: courseId, coursework_id: id }))
    }
    return this.request('/classroom/submissions/status/', {
      method: 'POST',
      body: JSON.stringify(payload)
